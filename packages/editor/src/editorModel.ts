...
export type EditorVisual =
  ...
  | { kind: "shape"; shape: ShapeNode["shape"]; fill?: string; stroke?: string; strokeWidth?: number; radius?: number }
  | { kind: "image"; src: string; fit?: ImageNode["fit"]; alt?: string; placeholder?: string }
  | { kind: "code"; codeType: "barcode" | "qr"; value: string; format?: string; placeholder?: string }
  | { kind: "container"; label: string; tone: "flow" | "repeat" | "group" | "stack" | "conditional" | "grid" };
...
export function buildEditorPageModel(template: DocumentTemplate, pageId?: string): EditorPageModel {
...
export function collectPageNodeItems(template: DocumentTemplate, pageId?: string): EditorNodeItem[] {
  const page = findPage(template, pageId);
...
export function updateNodeById(template: DocumentTemplate, nodeId: string, update: (node: EditableNode) => void): boolean {
...
export function updateNodesById(template: DocumentTemplate, updates: Record<string, Partial<Frame>>): boolean {
  let changed = false;
...
export function getAlignmentFramePatches(
  ...
  const selected = selectedIds.map((id) => subjects.find((subject) => subject.id === id)).filter((subject): subject is AlignmentSubject => Boolean(subject));

  if (selected.length === 0) {
    return {};
  }

  if ((command === "distribute-x" || command === "distribute-y") && selected.length < 3) {
    return {};
  }

  const bounds = selected.length === 1 ? { x: 0, y: 0, width: pageSize.width, height: pageSize.height } : getBounds(selected.map((subject) => subject.absoluteFrame));
  const patches: Record<string, Partial<Frame>> = {};

  if (command === "distribute-x") {
    const sorted = [...selected].sort((a, b) => a.absoluteFrame.x - b.absoluteFrame.x);
    ...
    const totalWidth = sorted.reduce((sum, subject) => sum + subject.absoluteFrame.width, 0);
    const gap = (last.absoluteFrame.x + last.absoluteFrame.width - first.absoluteFrame.x - totalWidth) / (sorted.length - 1);
    let nextX = first.absoluteFrame.x;

    for (const subject of sorted) {
      patches[subject.id] = { x: subject.frame.x + (nextX - subject.absoluteFrame.x) };
  ...
  if (command === "distribute-y") {
    const sorted = [...selected].sort((a, b) => a.absoluteFrame.y - b.absoluteFrame.y);
    const first = sorted[0];
    const last = sorted[sorted.length - 1];
    ...
    const totalHeight = sorted.reduce((sum, subject) => sum + subject.absoluteFrame.height, 0);
    const gap = (last.absoluteFrame.y + last.absoluteFrame.height - first.absoluteFrame.y - totalHeight) / (sorted.length - 1);
    let nextY = first.absoluteFrame.y;

    for (const subject of sorted) {
      patches[subject.id] = { y: subject.frame.y + (nextY - subject.absoluteFrame.y) };
  ...
  for (const subject of selected) {
    ...
    if (command === "align-center-x") {
      patch.x = subject.frame.x + (bounds.x + bounds.width / 2 - (subject.absoluteFrame.x + subject.absoluteFrame.width / 2));
    }

    if (command === "align-right") {
      patch.x = subject.frame.x + (bounds.x + bounds.width - (subject.absoluteFrame.x + subject.absoluteFrame.width));
    }
    ...
    if (command === "align-center-y") {
      patch.y = subject.frame.y + (bounds.y + bounds.height / 2 - (subject.absoluteFrame.y + subject.absoluteFrame.height / 2));
    }

    if (command === "align-bottom") {
      patch.y = subject.frame.y + (bounds.y + bounds.height - (subject.absoluteFrame.y + subject.absoluteFrame.height));
    }
...
function findPage(template: DocumentTemplate, pageId?: string): PageTemplate {
  return template.pages.find((page) => page.id === pageId) ?? template.pages[0] ?? emptyPage();
}
...
function renderNodeCollection(nodes: EditableNode[], context: RenderContext): EditorRenderNode[] {
  return nodes.flatMap((node, index) => renderNode(node, context, `${context.parentPath}.${node.id || index}`));
}

function renderNode(node: EditableNode, context: RenderContext, path: string): EditorRenderNode[] {
  ...
  if (node.type === "group") {
    return renderGroupLikeNode(node, context, path, "group", "Group", node.children);
  }

  if (node.type === "stack") {
    return renderGroupLikeNode(node, context, path, "stack", "Stack", node.children);
  }

  if (node.type === "conditional") {
    return renderGroupLikeNode(node, context, path, "conditional", "Condition", node.children);
  }
  ...
  return [createLeafNode(node, context, offsetFrame(node.frame, context.origin))];
}

function renderFlowChildren(nodes: FlowNode[], context: RenderContext): FlowLayoutResult {
  ...
  for (const [index, node] of nodes.entries()) {
    ...
    rendered.push(
      ...renderNode(node, { ...context, origin: nodeOrigin }, `${context.parentPath}.${node.id || index}`)
    );
    cursorY += node.frame.y + nodeHeight;
  }
...
function renderRepeatNode(node: RepeatNode, context: RenderContext, path: string): EditorRenderNode[] {
  ...
  return [
    createContainerNode(node, context, absoluteFrame, "repeat", `Repeat: ${node.binding.path}`),
    ...renderNodeCollection(node.children, childContext)
  ];
}

function createLeafNode(node: EditableNode, context: RenderContext, absoluteFrame: Frame): EditorRenderNode {
  return {
...
function createVisual(node: EditableNode, assets: Map<string, AssetDefinition>): EditorVisual {
  if (node.type === "text") {
    return {
      kind: "text",
  ...
  if (node.type === "barcode" || node.type === "qr") {
    return {
      ...
      placeholder: dynamicValueHasBinding(node.value) ? resolveDynamicValue(node.value) : undefined
    };
  }
...
function collectNodes(
  nodes: EditableNode[],
  context: Pick<EditorNodeItem, "pageId" | "layerId" | "layerKind" | "depth"> & { parentPath: string; origin: Pick<Frame, "x" | "y"> }
): EditorNodeItem[] {
  return nodes.flatMap((node, index) => {
    ...
    const children = getChildCollections(node).flatMap((childNodes, childIndex) =>
...
function collectFlowNodes(
  ...
  for (const [index, node] of nodes.entries()) {
    ...
    cursorY += node.frame.y + measureEditorFlowNodeHeight(node);
  }

  return items;
}

function updateNodeInCollection(nodes: EditableNode[], nodeId: string, update: (node: EditableNode) => void): boolean {
...
function updateNodeFramesInCollection(nodes: EditableNode[], updates: Record<string, Partial<Frame>>): boolean {
  let changed = false;
...
function measureChildrenBottom(children: Array<DocNode | FlowNode>): number {
  return children.reduce((bottom, child) => Math.max(bottom, child.frame.y + measureNodeHeight(child)), 0);
}

function measureNodeHeight(node: DocNode | FlowNode): number {
  if (node.type === "group" || node.type === "flowRegion" || node.type === "conditional") {
    return Math.max(node.frame.height, measureChildrenBottom(node.children));
  }
...
function measureStackHeight(node: StackNode): number {
  if (node.direction === "horizontal") {
    return measureChildrenBottom(node.children);
  }

  return node.children.reduce((height, child, index) => height + (index === 0 ? 0 : node.gap) + measureNodeHeight(child), 0);
}
...
function resolveImageSource(source: ImageSource, assets: Map<string, AssetDefinition>): { src: string; placeholder?: string } {
  if (source.kind === "url") {
...
function dynamicValueHasBinding(value: DynamicValue): boolean {
  ...
  return value.kind === "template" && value.parts.some((part) => part.kind === "field");
}
...
function editorFrameForNode(node: EditableNode, origin: Pick<Frame, "x" | "y">): Frame {
