...
export function resolvePageInspectorDraft(uiState: InspectorUiState, pageId: string, template?: DocumentTemplate): ResolvedPageInspectorDraft {
  ...
  return {
    ...
    // Layout aids default OFF so authors see a clean template (embedding field-test §1.14).
    showMarginGuides: draft.showMarginGuides ?? false,
    showPrintableArea: draft.showPrintableArea ?? false,
    pageShadow: draft.pageShadow ?? false,
    safeAreaEnabled: draft.safeAreaEnabled ?? false,
    safeAreaMm: draft.safeAreaMm ?? 24,
    bleedEnabled: draft.bleedEnabled ?? false,
    bleedMm: draft.bleedMm ?? 3,
    backgroundColor: draft.backgroundColor ?? "#FFFFFF",
    defaultFormat: draft.defaultFormat ?? "pdf",
    dpi: draft.dpi ?? "300",
    includeCropMarks: draft.includeCropMarks ?? false,
