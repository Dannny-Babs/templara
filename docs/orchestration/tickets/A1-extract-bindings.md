...
- [ ] `extractBindings` exported from `@templara/core`.
- [ ] Unit tests cover the node kinds listed above (at least field, repeat, conditional/visibleIf, formula path, image binding).
- [ ] Output is unique and sorted; empty template → `[]`.
- [ ] Changeset present for `@templara/core`.
- [ ] `pnpm typecheck && pnpm test && pnpm build` green.
- [ ] `pnpm run release:check` green if that is the publishable-package gate used in this repo.
