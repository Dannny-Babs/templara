---
title: "@templara/pdf"
description: Browser-first PDF export helpers for rendered Templara previews.
---
...
| Export | Purpose |
| --- | --- |
| `collectExportDiagnostics()` | Pure export preflight from `RenderDocumentResult` |
| `exportPreviewToPdf()` | Browser-first PDF export from rendered page elements |
| `buildExportFontCss()` | Builds font CSS for the isolated export surface |
...
renderDocument()
  -> DocumentPreview
  -> query [data-templara-page-id] elements
  -> collectExportDiagnostics()
  -> exportPreviewToPdf()
```

Do not pass raw template JSON directly to `@templara/pdf`; it expects rendered preview pages.

Related: [Validation & diagnostics](/docs/core-concepts/validation-and-diagnostics).
